...
@mcp.prompt(
    name="email_agent_persona", 
    description="Returns a short persona prompt for an email agent who loves food and dislikes exercise."
)
...
@mcp.tool(
    ...
    description="Read unread emails from Gmail inbox, returning a list of dicst: sender, subject, and body for each."
)
def read_unread_emails(max_results: int = 5) -> dict[str, list[dict[str, str]]]:
    """
    Read unread emails from Gmail inbox.
    Returns a list of simple dicts: from_address, subject, body
    """
    results = []
...
@mcp.tool(
    name="write_reply", 
    description="Use the email agent persona, received email body and LLM to write a reply"
)
def write_reply(original_email_body: str, prompt: str = None) -> dict[str, str]:
    ...
    reply_text = _generate_reply(
        email_body=original_email_body,
        persona_instructions=prompt
    )
...
@mcp.tool(
    name="send_email", 
    description="Send an email via Gmail SMTP using to_address, subject and body"
)
